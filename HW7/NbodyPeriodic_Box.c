...
double Dim[D];
double x0[N][D],v0[N][D],M[N],corner[8][D];
double x[N][D],v[N][D];
double dt=0.01,t=0;
// GUI variables
double xcenter=0,ycenter=0,Scale=100;
double screend=1,objd=2, R=0.3,tet=0;
...
void drawvolume(int xdim, int ydim){
    for (int d=0; d<D; d++) Dim[d]=10;
    for (int x=0; x<2; x++)
      for (int y=0; y<2; y++)
      for (int z=0; z<2; z++){
	corner[x+2*(y+2*z)][0]=(x-0.5)*Dim[0];
	corner[x+2*(y+2*z)][1]=(y-0.5)*Dim[1];
	corner[x+2*(y+2*z)][2]=(z-0.5)*Dim[2];	
      }

    double ct=cos(tet);
    double st=sin(tet);
    for (int n=0; n<8; n++){
      double ztmp= ct*corner[n][2]+st*corner[n][0];
      double xtmp=-st*corner[n][2]+ct*corner[n][0];
      corner[n][0]=xtmp;
      corner[n][2]=ztmp;
    }
    int xc[8], yc[8];
    for (int n=0; n<8; n++){
      xc[n]=xdim/2+Scale*screend*corner[n][0]/(corner[n][2]+objd);
      yc[n]=ydim/2-Scale*screend*corner[n][1]/(corner[n][2]+objd);
    }
    for(int n=0;n<8;n+=2)
    mydrawline(1,xc[n],yc[n],xc[n+1],yc[n+1]);
    for(int n=0;n<4;n++)
    mydrawline(1,xc[n],yc[n],xc[n+4],yc[n+4]);
    for(int n=0;n<6;n++){
      if(n==2)n+=2;
      mydrawline(1,xc[n],yc[n],xc[n+2],yc[n+2]);
    }
}
...
void initconfig(){
  ...
  for (int x=0; x<2; x++)
    for (int y=0; y<2; y++)
      for (int z=0; z<2; z++){
	x0[x+2*(y+2*z)][0]=x-0.5+Dim[0]/2;
	x0[x+2*(y+2*z)][1]=y-0.5+Dim[1]/2;
	x0[x+2*(y+2*z)][2]=z-0.5+Dim[2]/2;	
      }
  for (int n=0; n<N; n++){
    M[n]=1;
    for (int d=0; d<D; d++){
      v0[n][d]=(double)rand()/RAND_MAX;
    }
  }
}
...
int main(){
  ...
  if (D==3){
    ...
    AddFreedraw("Enclosed Volume",drawvolume);
  }
  StartMenu("Planets",1);
  StartMenu("Init",0);
  for (int n=0; n<N; n++){
