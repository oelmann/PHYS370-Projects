...
int main(){
  ...
  sprintf(name,"HarmonicDt%f_k%f_2nd.dat",dt,k);
  res=fopen(name,"w");
  for (t=0; t<totaltime; t+=dt){
    fprintf(res,"%f %f  %f %e\n",t,x[0],v[0],errorx);//,errorv);
  ...
  sprintf(name,"HarmonicDt%f_k%f_1st.dat",dt,k);
  res=fopen(name,"w");
  for (t=0; t<totaltime; t+=dt){
    fprintf(res,"%f %f  %f %e\n",t,x[0],v[0],errorx);//,errorv);
    errorx=sum2errorx(x,t,om,errorx);
  ...
  sprintf(name,"HarmonicDt%f_k%f_Analytical.dat",dt,k);
  res=fopen(name,"w");
  for (t=0; t<totaltime; t+=dt){
