...
void iterate(farr f,farr ff){
  for (int x=0; x<xdim; x++)
	...
	for (int j=0; j<3; j++){
	  ...
	  if(y==ydim-1) ff[i][j][(x+i-1+xdim)%xdim][(y+j-1+ydim)%ydim]=0*w[i][j];
	}
    }
  t+=1;
}
...
void init(){
  ...
  for (int x=0; x<xdim; x++)
    for (int y=0; y<ydim; y++){
      rho[x][y]=100;
      if (y-ydim/10<0) rho[x][y]=20;
      if (y-9*ydim/10>0) rho[x][y]=0;if (y-ydim/10<0) rho[x][y]=20;
      if (y-9*ydim/10>0) rho[x][y]=0;
    }
  for (int x=0; x<xdim; x++)
    for (int y=0; y<ydim; y++){
      for (int i=0; i<3; i++)
